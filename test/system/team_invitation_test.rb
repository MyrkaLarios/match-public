...
class TeamInvitationTest < ApplicationSystemTestCase
  ...
  test 'user can invite someone with an email different from @michelada.io when creating a team' do
    user = users(:user)
    sign_in user
  ...
  test 'user can invite someone with an email different from @michelada.io through invitation view' do
    user = users(:user)
    sign_in user
